function postComment(){
    $('#button_post_comment').on('click', function replaceWords(e) {
        ...
        var badWords = ["gay", "sex", "porno", "pula", "pizda", "lgbt", ""];
        ...
        $.post('/post_comment_seven', {com_name: selected_name, com_email: selected_email, com_text: remove_links}, function(data) {
            console.log(data);
        });
...
function getMostCommonWords() {
    ...
    $.get('/get_most_common_words_seven', function (data) {
		var words = [];
		var no_words_app = [];
		console.log(data);
